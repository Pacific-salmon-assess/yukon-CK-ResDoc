...
data{
  ...
  matrix [nyrs, A] fem_fec_comps;      // female spawners observed average fecundity by age class
}
...
transformed parameters{
  ...
  matrix<lower=0>[nyrs, A] E_A;         // observed total eggs by age class
  vector[nyrs] lnE;                     // log total egg states
  ...
  for (t in 1:nyrs) {
    for(a in 1:A){
      E_A[t,a] = (S[t]*(fem_S_comps[t,a]))*fem_fec_comps[t,a];
    }
    lnE[t] = log(sum(E_A[t,]));
  }  
  ...
  for (y in (A+a_min):nRyrs) { // Why start at 8? is this the first fully linked year?
    lnRm_1[y] = lnE[y-a_max] + lnalpha - beta * exp(lnE[y-a_max]); 
