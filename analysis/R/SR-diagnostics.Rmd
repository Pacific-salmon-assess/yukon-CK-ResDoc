...
for(i in unique(sp_har$CU)){
  ...
  p <- mcmc_combo(AR1.eggs.fits[[i]], pars = c("D_scale"),
             combo = c("dens_overlay", "trace"),
             gg_theme = legend_none())|> 
    as.ggplot() +
...
p <- mcmc_combo(TVA.eggs.fits[[i]], pars = c("D_scale"),
                combo = c("dens_overlay", "trace"),
                gg_theme = legend_none())|> 
  as.ggplot() +
...
We aimed for minimum effective sample sizes that are greater than ***400*** and $\hat{R}$ values less than 1.1.
