...
sim.pars.process <- sim.pars |>
  ...
  mutate(across(everything(), ~ifelse(. < 0.01, formatC(., format="e", digits=2), .))) |>
  ...
  mutate(CU_id = if_else(grepl("^alpha|beta", par),
                         gsub("\\D", "", par), NA)) |>
  left_join(data.frame(CU_id=as.character(1:9), CU=CU_name_lookup$CU_pretty)) |>
  mutate(par = gsub("\\_\\d$", "", par)) |>
  select(CU, par, value)
...
sim.pars.process |>
  ...
  dplyr::mutate_all(function(x){gsub("%", "\\\\%", x)}) |>
  dplyr::mutate_all(function(x){gsub("\\\\n","\n", x)}) |>
  dplyr::mutate_all(function(x){gsub("_", "\\\\_", x)}) |>
  csasdown::csas_table(format = "latex",
