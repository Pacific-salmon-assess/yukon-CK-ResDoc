...
(ref:fig-map-assess) Major assessment projects for adult Chinook salmon in the Canadian portion of the Yukon River watershed. A full list of assessment projects is provided in Table 3.
...
cugsitable |> 
  ...
  csasdown::csas_table(format = "latex",
                       ...
                       repeat_header = TRUE) |>
  kableExtra::column_spec(1, width = "0.75cm")|>
...
ref.df <- data.frame(Metric = c("Limit Reference Point (LRP) ", 
                       ...
                       Value = c("One or more component CUs for the MSU in Wild Salmon Policy Red status zone",
                                 ...
                                 "37%; Harvest rate associated with MSY ($U_{MSY}$) for the least productive CU in the SMU",
...
(ref:demo-ref-pts) Biological benchmarks based on estimated egg mass-recruitment relationships by Conservation Unit. Values are in thousands of fish, and shown are reference points assuming different age, size and sex composition observed in the first six years of time series ("Early"), last six years of time series ("Recent"), or averaged over all years .
...
HCR.df <- data.frame(HarvestControlRule = c("Moratorium", 
                        ...
                        Description = c("Current agreement between ADF&G and DFO (aka 'moratorium') that limits directed harvest unless border passage is expected to exceed 71,000 fish, whcih for simplicity is operatioanlized as a forecasted run-size that exceeds 71,000 \n", 
                                     ...
                                     "Current moratorium unless run-sizes are forecast to exceed 71,000 fish in which case there is a harvest rate cap at large run-sizes that is equal to $U_{MSY}$ of the least productive Conservation Unit (~37%) \n", 
...
\clearpage
(ref:tab-ref-sim-params) Parameters upon which closed-loop simulation model to assess impact of possible future fishery management measures was conditioned. $alpha$ values are those under the reference scenario (most recent generation productivity; 2013-2018), and $alpha$ values for alternative (robustness) productivity scenarios are found in Table \@ref(tab:OM-scenarios).
...
sim.pars.process$par[sim.pars.process$par == "pi_1"]<- "$\\pi_1$"
sim.pars.process$par[sim.pars.process$par == "pi_2"]<- "$\\pi_2$"
sim.pars.process$par[sim.pars.process$par == "pi_3"]<- "$\\pi_3$"
sim.pars.process$par[sim.pars.process$par == "pi_4"]<- "$\\pi_4$"
...
names(sim.pars.process) <- c("Conservation Unit", "Simulation Parameter", "Median (50$^{\\text{th}}$ percentiles) or value")
