...
(ref:fig-map-gsi) Genetic sampling locations and number of samples per site in the Canadian portion of the Yukon River watershed.
```{r fig-map-gsi, fig.cap = "(ref:fig-map-gsi)"}
knitr::include_graphics(here::here("csasdown/figure/yukon-map-gsi.png"))
```

(ref:fig-map-assess) Assessment sites in the Canadian portion of the Yukon River watershed. Eagle sonar is operated jointly by ADFG and DFO.
...
(ref:fig-prod-scenarios) Alternative productivity scenarios assessed in the closed-loop simulation model. Under the reference scenario, the 10-year average of recent $\alpha$ (year-year) are used to project the system forward in time. Under the robustness scenario, the most recent $\alpha$ (year) is used.
```{r fig-prod-scenarios, fig.cap = "(ref:fig-prod-scenarios)"}
knitr::include_graphics(here::here("analysis/plots/OM-productivity-scenarios.png"))
```

(ref:fig-hcrs) Illustration of harvest control rules considered in analysis of impacts of fishery management measures. We considered the rule used prior to the transboundary moratorium agreement (status quo) and a variation of it that allows only a 40% exploitation rate (status quo cap), as well as the current rule under the transboundary moratorium agreement and the same variation (moratorium and moratorium cap), as well as an alternative form of harvest control rule. The x-axis range of the plot corresponds approximately to the historical range of run sizes.

```{r fig-hcrs, fig.cap = "(ref:fig-hcrs)"}
knitr::include_graphics(here::here("analysis/plots/HCR_visualize.png"))
```

\clearpage

(ref:fig-perf-ref) Performance of alternative harvest control rules in the 'reference' projected fishery scenario. (a) Median (coloured bars) and 50% quantiles (vertical lines) of performance measures calculated over the years 2030-2050 and 1000 simulated fishery scenarios. (b) Mean number of CUs that fall above, below, and between stock reference points (20% $S_{MSR}$ and $S_{MSR}$) in the final simulated year (2050), over 1000 iterations.

```{r fig-perf-ref, fig.cap = "(ref:fig-perf-ref)"}
knitr::include_graphics(here::here("analysis/plots/perf_metrics_status_TVA.png"))
```

\clearpage

(ref:fig-ER-tradeoff-ref) Performance of a range of fixed exploitation rates in the 'reference' projected fishery scenario. (a) Mean number of CUs that fall above, below, and between stock reference points (20% $S_{MSR}$ and $S_{MSR}$) in the final simulated year (2050), over 1000 iterations. (b) Mean spawners by CU under a range of exploitation rates, over all years of simulation (2025-2050).

```{r fig-ER-tradeoff-ref, fig.cap = "(ref:fig-ER-tradeoff-ref)"}
knitr::include_graphics(here::here("analysis/plots/fixed_ER_tradeoffs_TVA.png"))
```
...
priors.df <- data.frame(Reference = c("Limit Reference Point (LRP)", 
                        ...
                        Scale = c("Stock Management Unit", 
                           ...
                           Definition = c("CU-status based; an SMU is below its LRP when one or more component CUs are in the WSP Red status zone. This definition is aligned with principle that LRPs be aligned with Canada’s WSP objective of preserving biodiversity of salmon at the scale of CUs [@dfo2022limit; @holt2023guidelines]. However, it is difficult to operationalize this type of LRP for fisheries management, and so for management purposes an LRP-Lower may also be required. ",
                                          ...
                                          "Differentiates amber and red Wild Salmon Policy status zones. Assessing WSP rapid statuses considers multiple metrics, and this is one metric that is applied if the data and related benchmarks are available. When it can be estimated SGEN is often used and it is the spawning abundance that is expected to lead to recovery to $S_{MSY}$ in one salmon generation in the absence of fishing under equilibrium conditions [@holtEvaluationBenchmarksConservation2009]. Spawning abundances below this reference point (red status) are expected to be associated with high risk of irreversible harm to the CU and align with a COSEWIC assessment of “endangered”. ",
                                          "The spawning abundance that differentiates amber and green Wild Salmon Policy status zones. When it can be estimated 80% $S_{MSY}$ is used and is the spawner abundance expected to provide, on average annual basis, the maximum annual catch for the CU, given existing environmental conditions and where “there would not be a high probability of losing the CU” [@WSP2005].",
                                          "Other biologically based reference points like $S_{MSR}$, which is the spawner abundance expected to maximize returns on average annual basis, may be used as an upper biological reference point and 20% of $S_{MSR}$ which has been proposed as a lower biological reference point. $S_{MSR}$ is sometimes referred to as $S_{MAX}$, and escapement goals based on this quantity are sometimes deemed more appropriate where the fishery is dominated by subsistence fisheries that wish to harvest a fixed number of fish each year and are attempting to minimize the effort needed to harvest [@liller2022]."))
...
(ref:pars-ref-pts) Conservation Unit biological reference points. Values are in thousands of fish.
...
(ref:pars-sr) Conservation Unit stock-recruit parameters.
...
demo.pars$Benchmark[demo.pars$Benchmark == "Umsy"]<- "$U_{\\text{msy}}$"
...
\clearpage
(ref:hcrs) Definitions of alternative fishery management measures (Harvest Control Rules) considered.

```{r hcrs, results = "asis"}
HCR.df <- data.frame(HarvestControlRule = c("Moratorium", 
                                               "Status quo (Interim Management Escapement Goal)", 
                                               "Moratorium w/ harvest cap", 
                                               "Status quo w/ harvest cap", 
                        ...
                        Description = c("Current moratorium on all directed harvest unless run-sizes are forecast to exceed 71,000 fish", 
                                     "Interim Management Escapement goal that was in place prior to the current moratorium. At forecasts below 42,500 fish no directed harvest and target harvest equal to 'surplus' if forecast exceeds escapement goal", 
                                     ...
                                     "Harvest Control Rule that is consistent with DFO's Precautioanry Approach Framework: no harvest below lower fishery reference point [19,000] with progressive increase in allowable harvest rate up to maximum equal to $U_{MSY}$ of the least productive Conservation Unit when run-sizes reach spawner abundances associated with the Upper Stock Reference Point [95,000]"))
