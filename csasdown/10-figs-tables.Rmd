...
(ref:fig-CU-comps) Percentage contribution of each Conservation Unit to reconstructed total spawner abundance of the Yukon Chinook salmon Stock Management Unit over time. Note these estimates of percent contribution differ from the raw values in Table \@ref(tab:tab-CU-gsi-annual-summary) because they are derived from a run-reconstruction model that explicitly accounts for missing years and the sometimes unrepresentative GSI sampling of total returns. 
...
(ref:ref-points) Definitions of some common biological reference points used for salmon at either the Stock Management Unit (SMU) or Conservation Unit (CU) scale.
...
(ref:OM-scenarios) Alternative productivity ($alpha$) scenarios tested in forward simulations.
