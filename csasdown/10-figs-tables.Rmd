...
(ref:fig-map-assess) Sites of major assessment projects for adult Canadian-origin Yukon Chinook that may inform in-season management decisions. Several smaller-scale assessment projects and aerial surveys exist that are not included.
...
(ref:fig-prod-trends) Trends in intrinsic productivity (ricker $\alpha$ parameter; maximum recruits per spawner) over time by CU as predicted by time-varying productivity models.

```{r fig-prod-trends, fig.cap = "(ref:fig-prod-trends)"}
knitr::include_graphics(here::here("csasdown/figure/changing_productivity.png"))
```

(ref:fig-asl) Age, sex, and length trends in Canadian-origin Yukon River Chinook salmon over time. a) sex ratio, represented as proportion of spawners that are female, b) body length of female spawners ! DEFINE MEFL?!, c) proportion of returning spawners in each age class over time, d) Average reproductive output of female spawners (eggs per female spawner).

```{r fig-asl, fig.cap = "(ref:fig-asl)"}
knitr::include_graphics(here::here("csasdown/figure/asl.png"))
```

(ref:fig-CU-EMR) Predicted egg mass-recruitment relationships by CU (black lines) with uncertainty (80% credible intervals; shaded areas). Points are data inputs to egg mass models 
...
(ref:fig-em-vs-spw-sr-TVA) Scaled time trends in recruits-per-reproductive unit across alternative SR models.
...
(ref:fig-demo-smsr) Biological benchmarks (S$_{MSR}$) as calculated from egg mass-recruit models (yellow) and spawner-recruit models (purple).

```{r fig-demo-smsr, fig.cap = "(ref:fig-demo-smsr)"}
knitr::include_graphics(here::here("csasdown/figure/demo_bench_compare.png"))
```
...
(ref:fig-projections) Projected spawners over time from closed-loop simulation models conditioned on the reference (10-year $\alpha$) scenario. Three of six harvest control rules are shown.
...
(ref:assess-proponents) Assessment projects which provide in-season fisheries data on Chinook salmon returns to Canadian Yukon River and tributaries. 
```{r assess-proponents, results = "asis"}

assess_tbl <- read.csv(file=here::here("csasdown/data/trib-assessment-propnents.csv"))
...
assess_tbl |> drop_na() |> 
  csasdown::csas_table(format = "latex",
                       ...
                       repeat_header = TRUE,
                       align = "llc") |>
  kableExtra::add_footnote(label=c("* Tatchun Creek assessements were conducted from 1970-2000 using foot surveys, aerial surveys or weir")) |>
  kableExtra::column_spec(2, width = "3cm")
...
priors.df <- data.frame(Reference = c("Limit Reference Point (LRP)", 
                        ...
                        Scale = c("Stock Management Unit", 
                           ...
                           Definition = c("CU-status based; an SMU is below its LRP when one or more component CUs are in the WSP Red status zone. This definition is aligned with principle that LRPs be aligned with Canada’s WSP objective of preserving biodiversity of salmon at the scale of CUs (DFO 2022; Holt et al. 2023a). However, it is difficult to operationalize this type of LRP for fisheries management, and so for management purposes an LRP-Lower may also be required. ",
                                          ...
                                          "Differentiates amber and red Wild Salmon Policy status zones. Assessing WSP rapid statuses considers multiple metrics, and this is one metric that is applied if the data and related benchmarks are available. When it can be estimated SGEN is often used and it is the spawning abundance that is expected to lead to recovery to $S_{MSY}$ in one salmon generation in the absence of fishing under equilibrium conditions (Holt 2009). Spawning abundances below this reference point (red status) are expected to be associated with high risk of irreversible harm to the CU and align with a COSEWIC assessment of “endangered”. ",
...
\clearpage
\begin{landscapepage}
(ref:pars-ref-pts) Conservation Unit biological reference points. Values are in thousands of fish (except $U_{MSY}$).
...
demo.pars <- demo.pars |> 
  dplyr::mutate(period = dplyr::case_when(period == "avg" ~ "Average (all years)",
                                          period == "early" ~ "Early (1985-1989)",
                                          period == "recent" ~ "Recent (2019-2024)")) |>
  mutate_at(4:6, function(x){round(x/1000, 2)}) |>
  dplyr::mutate(`Median (95$^{\\text{th}}$ percentile)` = paste0(median, " (", lower, ",", upper, ")"))
...
\clearpage
\begin{landscapepage}
(ref:tab-ref-perf-measures) Performance measures from "reference set" simulations, where productivity returns to its 10-year average. Escapement and harvest values are in thousands of fish.
...
names(ref_case_pms) <- c("Harvest Control Rule", "Metric", "Mean", "Median (50$^{\\text{th}}$ percentile)")
...
\clearpage
(ref:tab-rob-perf-measures) Performance measures from "robustness set" simulations, where productivity remains at 2024 levels. Escapement and harvest values are in thousands of fish.
...
names(rob_case_pms) <- c("Harvest Control Rule", "Metric", "Mean", "Median (50$^{\\text{th}}$ percentile)")
