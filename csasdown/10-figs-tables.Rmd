...
(ref:map-gsi) Various assessment sites and genetics sampling locations and number of samples per site in the Canadian portion of the Yukon River watershed.
...
demo.pars <- dplyr::mutate(demo.pars, period = case_when(period == "avg" ~ "Average (all years)",
                                                         period == "early" ~ "Early()",
...
ref_case_pms <- read.csv(here::here("analysis/data/generated/perf_metrics_TVA.csv")) |>
  pivot_longer(cols=2:10, values_to="value", names_to="metric") |>
...
ref_case_pms <- ref_case_pms |> 
  filter(!(HCR %in% fix_hcr))  # filter out fixed harvest 
