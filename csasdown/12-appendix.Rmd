...
\end{equation}
	...
	μ_y=μ_(y-1) exp (ε_y )       
(Eqn. S1)
where ε_y is a vector of nine normally distributed process errors with mean 0 and covariance Σ. The covariance matrix is constructed as Σ=DCD, where D is a diagonal matrix with the variance of μ_s as the ssth element, and C is a symmetric correlation matrix, i.e.,
	C_ij={■(1&i=j@c_ij&i≠j)┤  	(Eqn. S2)
where c_ij is the correlation between arrival timing deviations for population i and j (c_ij=c_ji ). We constrained all off-diagonal elements of C to have the same value, which assumes all stocks have equal correlation in arrival timing deviations.
