...
(ref:tab-rr-params) Model notation for multi-CU run-reconstruction model. **Needs to be updated**
...
\begin{landscapepage}
(ref:tab-rr-eqns) Model equations for multi-CU run-reconstruction model.**Needs to be updated**
...
For each population, the daily proportion of salmon entering the model (i.e., crossing the US-Canada border into CDN waters) in a given year is calculated based on a normal probability density function, with mean $\mu_{sy}$ with variance $\sigma^2_s$, which is divided by its sum to ensure that the individual values sum to 1 (Table \@ref(tab:tab-rr-eqns) 4.4). Numbers of salmon arriving daily for each population/year combination is subsequently calculated as the product of the daily arrival proportions and run size (Table \@ref(tab:tab-rr-eqns) 4.5). To model interannual variability in mean run timing, we assume that the population-specific mean dates of arrival in a given year (${\lbrace \mu_{s,y}\rbrace}_{s=1}^9$ or $\mu_{y}$) are a function of run timing in the previous year, i.e.:
...
In some years prior to the implementation of sonar monitoring, the fish wheel data was not informative enough to estimate run sizes for some CUs. To stabilize estimates in these cases, we applied a normal prior on the interannual deviations of log-scale run sizes for each CU (Table \@ref(tab:tab-rr-eqns) 16). The prior was set to 1, which is wide enough to have negligible effects in years for which data were sufficiently informative to estimate run size and was omitted from 2005 onward, after the introduction of sonar monitoring.
