...
In some years prior to the implementation of sonar monitoring, the fishwheel data was not informative enough to estimate border passage for some CUs. To stabilize estimates in these cases, we applied a normal prior on the interannual deviations of log-scale border passage for each CU (Table \@ref(tab:tab-rr-eqns).16). The prior was set to 1, which is wide enough to have negligible effects in years for which data were sufficiently informative to estimate border passage and was omitted from 2005 onward, after the introduction of sonar assessment of border passage.
...
(ref:tab-tva-pars) Estimates of key spawner-recruitment relationship parameters from time-varying productivity models by Conservation Unit along with two measures of parameter estimability and model convergence. Estimates of $\beta$ are on the scale of 10,000 fish for ease of summarizing parameters in the table. For $\alpha$ and $\sigma_\alpha$, which are time-varying parameters, values are averages of the posterior median estimated for each year along with the standard deviation among years.
...
pars <- pars |> filter(grepl("^beta", X)) |> 
  ...
  mutate(value = round(value*10000, 2)) |>
  ...
  mutate(n_eff = round(n_eff, 0), Rhat = round(Rhat, 3))
