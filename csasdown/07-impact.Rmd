...
The specific harvest control rules we considered are shown in Figure \@ref(fig:fig-hcrs) and described in Table \@ref(tab:hcrs). These alterantive fishery management measures ranged from a no fishing scenario to illustrate expected recovery potential, to fixed exploitation rate rules intended to simply illustrate the expected impact of a range of harvest rates, to escapement goal type rules (with and without caps on maximum exploitation) that were intended to approximate a range of realistic harvest management strategies. Conservation Units were assumed to be equally vulnerable to harvest and harvest control rules were defined by a combination of a lower management reference point and/or an upper reference point and a maximum removal reference, all defined at the stock aggregate scale. When there is both a lower and upper reference point, harvest rates linearly increase from zero at spawner abundances below the lower reference point to the maximum removal reference at run sizes equal to the spawner abundances associated with the upper reference point. The maximum removal reference was never allowed to exceed 80% in any scenarios because this is the maximum estimated aggregate harvest rate to have been experienced by the stock (occurred in 1987).
...
Under a "no fishing" scenario, median spawner abundances were projected to exceed CU specific lower reference points for all CUs by 2050, and two CUs were projected to exceed their upper reference points over the same time period: Middle Yukon and tribs., Nordenskiold, and Yukon R. Teslin Headwaters (Figure \@ref(fig:fig-perf-ref)).

Simulations across a range of fixed harvest rates highlighted a clear tradeoff between mixed-stock fishery harvests and risks to component CUs. Aggregate harvests are projected to be maximized at mixed-stock harvest rates of \~50%, and dominated by harvest from the Middle Yukon and Teslin headwaters CUs, but this comes at the cost of three least productive CUs being below their lower biological reference point and hence at an elevated level of extinction risk (Figure \@ref(fig:fig-ER-tradeoff-ref)).
